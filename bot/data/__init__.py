...
def _generate_aliases():
    ...
    with open(f'bot/data/aliases.txt', 'r') as f:
        for line in f:
            raw_aliases = line.strip().lower().split(',')
...
def _config():
    ...
    test_var = (
        config.AUTHORS, config.CATEGORY_ALIASES, config.ID_TYPE, config.PREFIXES, config.BOT_DESCRIPTION,
        config.GITHUB_IMAGE_REPO_URL, config.INVITE, config.SUPPORT_SERVER, config.BOT_SIGNATURE, config.ID_GROUPS,
        config.NAME, config.SOURCE_LINK, config.CATEGORY_NAME
    )
    test_var = None
